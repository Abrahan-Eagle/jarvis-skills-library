...
2. **SPAWN** a Task subagent (Cursor) using the SCANNER_PROMPT
3. Sub-agent writes to `~/.cursor/learning-captures/[session-id]/scan-NNN.md`
...
- Write to `~/.cursor/learning-captures/[session-id]/scan-NNN.md`
...
2. Create session directory:
   ```bash
   mkdir -p ~/.cursor/learning-captures/[session-id]
   ```

3. Determine scan number:
   ```bash
   ls ~/.cursor/learning-captures/[session-id]/scan-*.md 2>/dev/null | wc -l
   ```
...
FIRST: Check for scratch file at ~/.cursor/learning-captures/[session-id]/scratch.md
...
SECOND: Read ~/.cursor/learning-captures/watch-list.md in full (v3.9 May 12 2026).
...
WRITE OUTPUT to ~/.cursor/learning-captures/[session-id]/scan-NNN.md:
...
Active when Phase 1 shadow mode is in flight. Skip entirely if `~/.cursor/learning-captures/persona-eval-runs.txt` does not exist (Phase 1 not yet shipped or already past Phase 1).
...
1. **Read accumulators:**
   ...
   - Read `~/.cursor/learning-captures/phase-1-ship-date.txt` for Phase 1 ship date (set in Step 4c bootstrap)
   - Read `~/.cursor/learning-captures/phase-1-decision-log.md` if it exists — most recent decision marks where Phase 1 stands today
...
3. **If trigger condition met:**
   a. Spawn Phase 1 Decision Report sub-agent with `PHASE_1_DECISION_REPORT_PROMPT` (see prompt block alongside CONSOLIDATION_PROMPT). Pass it: all `~/.cursor/learning-captures/*/persona-eval.md` files, plus the `persona-eval-runs.txt` log.
   ...
   c. Capture user's decision in `~/.cursor/learning-captures/phase-1-decision-log.md`. Append in this format:
...
**Phase mode resolution** — read `~/.cursor/learning-captures/phase-1-decision-log.md` if it exists:
...
3. Both outputs written to `~/.cursor/learning-captures/[session-id]/persona-review.json` as a single combined file:
...
Before finalizing the Noted bucket: check `~/.cursor/learning-captures/watch-list.md`.
...
If `~/.cursor/learning-captures/[session-id]/persona-review.json` exists, render its contents into a per-conclusion table. The personas reviewed each routed conclusion. Their output is informational — user reads both views and decides per-row.
...
1. Read `~/.cursor/learning-captures/watch-list.md`.
...
**Companion file:** `~/.cursor/learning-captures/graduation-log.md` (created 2026-05-20).
...
Skip this step entirely if `~/.cursor/learning-captures/[session-id]/persona-review.json` does not exist (Phase 1 not active, REVERT in effect, or persona run failed).
...
3. **Write `~/.cursor/learning-captures/[session-id]/persona-eval.md`** in this exact YAML schema:
...
4. **Append a one-line entry to `~/.cursor/learning-captures/persona-eval-runs.txt`** with the format:
...
5. **Bootstrap-on-first-run note:** if this is the first Phase 1 wrap-up (file did not exist before this step), also write a sentinel file at `~/.cursor/learning-captures/phase-1-ship-date.txt` containing today's date in `YYYY-MM-DD` format. Step 1b.5 reads this to compute the 7-day-since-ship trigger.
...
rm ~/.cursor/learning-captures/[consolidated-session-id]/scan-*.md 2>/dev/null
rm ~/.cursor/learning-captures/[consolidated-session-id]/consolidation.md 2>/dev/null
rm ~/.cursor/learning-captures/[consolidated-session-id]/persona-review.json 2>/dev/null
rm ~/.cursor/learning-captures/[consolidated-session-id]/persona-eval.md 2>/dev/null
rm ~/.cursor/learning-captures/[consolidated-session-id]/scratch.md 2>/dev/null
rmdir ~/.cursor/learning-captures/[consolidated-session-id]/

# Verification (MANDATORY — confirms cleanup actually fired)
ls ~/.cursor/learning-captures/ | grep "[consolidated-session-id]" && echo "❌ CLEANUP FAILED — session dir still exists; re-run Step 6" || echo "✅ Session dir cleaned"
```
...
2. **Root cause check (in-session repetition AND cross-session via watch-list — Mod 2, Apr 28 2026):**
   ...
   **FIRST — read `~/.cursor/learning-captures/watch-list.md` end to end.** Every active entry has a `Root cause` column and a `Fix` column. These are the matching anchors.
...
A cluster in ~/.cursor/learning-captures/watch-list.md has matured (≥5 sub-IDs)
...
INPUTS (will be passed to you):
- All persona-eval.md files at ~/.cursor/learning-captures/*/persona-eval.md (the per-wrap-up eval data)
- The persona-eval-runs.txt log at ~/.cursor/learning-captures/persona-eval-runs.txt (one-line entries per shadow run)
...
```bash
mkdir -p ~/.cursor/learning-captures/[session-id] && echo "## Scratch Log (unverified micro-signals — input for scan/wrap-up, not a source of truth)\n---" > ~/.cursor/learning-captures/[session-id]/scratch.md
```

**Subsequent writes** (append one-liner):
```bash
echo "[ISO-timestamp] [source] Signal summary | Key detail or corrective action" >> ~/.cursor/learning-captures/[session-id]/scratch.md
```
    ...
    "SessionStart": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "if [ -d ~/.cursor/learning-captures ] && [ \"$(ls -A ~/.cursor/learning-captures 2>/dev/null)\" ]; then echo 'LEARNING_CAPTURES_EXIST: Found learning captures from before clear. Ask user if they want to review/consolidate before continuing.'; fi"
          }
        ]
      }
    ]
